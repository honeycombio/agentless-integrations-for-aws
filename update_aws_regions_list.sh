...
if [[ ${#REGIONS_WITHOUT_BUCKETS[@]} -gt 0 ]]; then
  ...
  echo ""
  echo "  # Make it public (required due to S3's new default settings)"
  echo "  aws s3api put-public-access-block \\"
  echo "    --bucket honeycomb-integrations-\${REGION} \\"
  ...
  echo ""
  echo "  aws s3api put-bucket-ownership-controls \\"
  echo "    --bucket honeycomb-integrations-\${REGION} \\"
  echo "    --ownership-controls=\"Rules=[{ObjectOwnership=BucketOwnerPreferred}]\""
  echo ""
  echo "  aws s3api put-bucket-acl \\"
  echo "    --bucket honeycomb-integrations-\${REGION} \\"
