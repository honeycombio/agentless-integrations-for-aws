...
const (
	...
	patternVPCFlow = `(?P<version>\d+) (?P<account_id>\d+) (?P<interface_id>eni-[0-9a-f]+) (?P<src_addr>[\d\.]+) (?P<dst_addr>[\d\.]+) (?P<src_port>\d+) (?P<dst_port>\d+) (?P<protocol>\d+) (?P<packets>\d+) (?P<bytes>\d+) (?P<start_time>\d+) (?P<end_time>\d+) (?P<action>[A-Z]+) (?P<log_status>[A-Z]+) (?P<vpc_id>vpc-[a-z0-9]+) (?P<subnet_id>subnet-[a-z0-9]+) (instance_id TODO) (?P<tcp_flags>\d+) (?P<type>IPv4|IPv6|EFA) (?P<pkt_srcaddr>[\d\.]+) (?P<pkt_dstaddr>[\d\.]+) (?P<region>[a-z0-9\-]+) (?P<az_id>[a-z0-9\-]+) (?P<sublocation_type>wavelength|outpost|localzone|-) (?P<sublocation_id>\d+ TODO) (?P<pkt_src_aws_service>[A-Z_0-9]+) (?P<pkt_dst_aws_service>[A-Z_0-9]+) (?P<flow_direction>ingress|egress) (?P<traffic_path>[1-8]) (?P<ecs_cluster_arn>[^ ]+) (?P<ecs_cluster_name>[A-Za-z0-9-_]+) (?P<ecs_container_instance_arn>[^ ]+) (?P<ecs_container_instance_id>[a-z0-9\-]+) (?P<ecs_second_container_id>[a-z0-9\-]+) (?P<ecs_service_name>[A-Za-z0-9\-_]+) (?P<ecs_task_definition_arn>[^ ]+) (?P<ecs_task_arn>[^ ]+) (?P<ecs_task_id>[a-z0-9\-]) (?P<reject_reason>BPA)`
