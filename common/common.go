...
const (
	...
	patternVPCFlow = `'(?P<version>\d+) (?P<account_id>\d+) (?P<interface_id>eni-[0-9a-f]+) (?P<src_addr>[\d\.]+) (?P<dst_addr>[\d\.]+) (?P<src_port>\d+) (?P<dst_port>\d+) (?P<protocol>\d+) (?P<packets>\d+) (?P<bytes>\d+) (?P<start>\d+) (?P<end>\d+) (?P<action>[A-Z]+) (?P<log_status>[A-Z]+)'`
